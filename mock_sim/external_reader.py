...
def get_module_base(process_id: int, module_name: str) -> Optional[int]:
    ...
    snapshot = kernel32.CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, process_id)
    if snapshot == ctypes.c_void_p(-1).value:
        return None
...
def world_to_screen(world_pos: Vec3, view_matrix: Mat4, screen_w: int, screen_h: int) -> Optional[Tuple[float, float]]:
    """
    ...
    clip_x = x * view_matrix[0][0] + y * view_matrix[0][1] + z * view_matrix[0][2] + view_matrix[0][3]
    clip_y = x * view_matrix[1][0] + y * view_matrix[1][1] + z * view_matrix[1][2] + view_matrix[1][3]
    clip_w = x * view_matrix[3][0] + y * view_matrix[3][1] + z * view_matrix[3][2] + view_matrix[3][3]
...
class CS2Reader:
    ...
    def update(self):
        ...
        # Local player
        local_addr = self.mem.read_uint32(self.mem.client_base + Offsets.dwLocalPlayerController)
        if not local_addr:
            return
        ...
        for i in range(1, 65):
            ...
            name = self.mem.read_string(controller_addr + Offsets.m_sSanitizedPlayerName)
            ...
            list_entry = self.mem.read_uint32(entity_list + 0x8 * (pawn_index >> 9) + 0x10)
            if not list_entry:
                continue
...
class CS2Overlay:
    def __init__(self, root: tk.Tk, reader: CS2Reader):
        ...
        self.view = tk.Canvas(root, width=VIEW_SIZE[0], height=VIEW_SIZE[1], bg="#101418")
    ...
    def _draw_radar(self):
        ...
        for p in self.reader.players:
            ...
            c.create_text(sx, sy - 10, text=str(int(p.health)), fill=color, font=("Consolas", 7))
